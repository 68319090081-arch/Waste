...
def random_rotate(image: np.ndarray, max_angle: float = 180.0) -> np.ndarray:
    ...
    ใช้เมื่อวัตถุในภาพไม่มีทิศทางตายตัว (เช่น ภาพถ่ายดาวเทียม, เซลล์, ชิ้นส่วนอุตสาหกรรม)
    ถ้าวัตถุมีทิศทางที่มีความหมาย (เช่น คน, รถ ที่ควรตั้งตรงเสมอ) ควรใช้มุมแคบกว่านี้ (เช่น ±20-30 องศา)
    """
    h, w = image.shape[:2]
...
def rotate_fixed_angle(image: np.ndarray, angle: float) -> np.ndarray:
    """หมุนภาพด้วยมุมที่กำหนดตายตัว (ไม่สุ่ม) ใช้สำหรับสร้างไฟล์แยกแต่ละองศา"""
    h, w = image.shape[:2]
    M = cv2.getRotationMatrix2D((w / 2, h / 2), angle, 1.0)
    return cv2.warpAffine(image, M, (w, h), borderMode=cv2.BORDER_REFLECT)
...
def augment_image(image: np.ndarray) -> np.ndarray:
    """รวม augmentation หลายแบบเข้าด้วยกัน (สุ่มแต่ละครั้ง) — ใช้เมื่อต้องการภาพ augment เดียวแบบสุ่ม"""
...
if __name__ == "__main__":
    # สคริปต์จะอ่านภาพทั้งหมดจากโฟลเดอร์ "data" ที่อยู่ระดับเดียวกับไฟล์นี้โดยอัตโนมัติ
    ...
    if not os.path.isdir(INPUT_DIR):
        ...
        print("กรุณาสร้างโฟลเดอร์ชื่อ 'data' ไว้ที่ parent directory ของสคริปต์นี้ แล้วใส่ภาพลงไป")
    else:
        preprocess_dataset(
            INPUT_DIR,
            OUTPUT_DIR,
